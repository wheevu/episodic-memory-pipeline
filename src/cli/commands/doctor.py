...
def _doctor_dry_run(use_mock: bool) -> None:
    ...
    table2 = Table(
        title="Provider Selection (Predicted)", show_header=True, header_style="bold cyan"
    )
    ...
    table2.add_row(
        "LLM", "mock" if pred["will_use_mock_llm"] else resolved["llm_provider"], llm_status
    )
    ...
    console.print()
    console.print(
        "[dim]Run without --dry to see full diagnostics with initialized components.[/dim]"
    )
...
def _doctor_full(ctx: click.Context, use_mock: bool) -> None:
    ...
    dim_status = "[green]✓ Match[/green]" if vs.dimension_match else "[red]✗ MISMATCH[/red]"
    ...
    if llm.is_mock:
        table5.add_row(
            "LLM", "[yellow]⚠ MOCK[/yellow]", "Fact/consolidation metrics may not be meaningful"
        )
    else:
    ...
    if not vs.dimension_match:
        ...
        table5.add_row(
            "Dimensions", "[green]✓ Consistent[/green]", f"All using {emb.dimension}d vectors"
        )
