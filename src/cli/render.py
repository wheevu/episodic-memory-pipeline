...
def render_eval_metrics(metrics: Dict[str, Any], verbose: bool = False) -> None:
    ...
    if fact_consistency:
        ...
        consistency_color = (
            "green" if consistency >= 0.9 else "yellow" if consistency >= 0.7 else "red"
        )

        table3.add_row(
            "Consistency Rate", f"[{consistency_color}]{consistency:.1%}[/{consistency_color}]"
        )
...
def render_eval_comparison(comparison: Any) -> None:
    ...
    if comparison.metric_diffs:
        ...
        table2.add_column("Metric")
        table2.add_column("Run A")
        table2.add_column("Run B")
