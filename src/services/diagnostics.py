...
class DiagnosticsService:
    ...
    def __init__(
        self, components: Optional["PipelineComponents"] = None, config: Optional["Config"] = None
    ) -> None:
        """
    ...
    def run_dry_diagnostics(self, force_mock: bool = False) -> Dict[str, Any]:
        ...
        # Predict mock usage
        will_use_mock_embeddings = force_mock or config.embedding_provider == "mock"
    ...
    def _get_llm_status(self) -> ProviderStatus:
        ...
        if isinstance(llm, MockLLMProvider):
            return ProviderStatus(
                type="mock", model="MockLLM (deterministic)", is_mock=True, temperature="N/A"
            )
            ...
            return ProviderStatus(
                type=llm.__class__.__name__, model="unknown", is_mock=getattr(llm, "is_mock", True)
            )
    ...
    def _get_eval_readiness(
        ...
        if not vector_store.dimension_match:
            warnings.append(
                f"Dimension mismatch → Vector search will fail! "
