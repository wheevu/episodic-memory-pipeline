...
def get_config_diagnostics(config: Config, force_mock: bool = False) -> ConfigDiagnostics:
    ...
    will_use_mock_embeddings = (
        force_mock or 
        resolved_embedding_provider == "mock"
    )
...
def generate_fix_suggestions(
    ...
    if force_mock:
        return ["# Mock mode explicitly enabled via --mock flag",
    ...
    if is_mock_embeddings:
        ...
        suggestions.append("export EMBEDDING_DEVICE=cpu  # or 'mps' for Apple Silicon, 'cuda' for NVIDIA")
        suggestions.append("")
