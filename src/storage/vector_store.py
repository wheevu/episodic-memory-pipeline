...
try:
    ...
    raise ImportError("faiss-cpu package required. Install with: pip install faiss-cpu") from err
...
class VectorStore:
    ...
    def add(self, index_name: str, record_id: str, embedding: np.ndarray) -> int:
    ...
    def add_batch(
        self, index_name: str, record_ids: list[str], embeddings: np.ndarray
    ) -> list[int]:
    ...
    def save(self, index_name: Optional[str] = None) -> None:
        ...
        for name in indices_to_save:
            ...
            try:
                faiss.write_index(self._indices[name], str(self._index_path(name)))
                np.save(str(self._id_map_path(name)), np.array(self._id_maps[name], dtype=object))
