...
class ClassificationResult:
    ...
    def not_worthy(cls, reason: str = "Not memory-worthy") -> "ClassificationResult":
        ...
        """
        return cls(is_memory_worthy=False, confidence=1.0, reason=reason, memory_type="none")
...
class MemoryWorthinessClassifier:
    ...
    def _compile_patterns(self) -> None:
        ...
        self._not_worthy_re = [re.compile(p, re.IGNORECASE) for p in self.NOT_WORTHY_PATTERNS]
        self._likely_worthy_re = [re.compile(p, re.IGNORECASE) for p in self.LIKELY_WORTHY_PATTERNS]
    ...
    def _llm_classify(
        self, text: str, previous_topic: Optional[str], context: Optional[str]
    ) -> ClassificationResult:
        ...
        prompt = PromptTemplates.MEMORY_WORTHINESS.format(
            text=text, previous_topic=previous_topic or "none", context=context or "none"
        )
        ...
        try:
            ...
            reason = (
                as_str(result.get("reason"), default="LLM classification") or "LLM classification"
            )
    ...
    def batch_classify(self, texts: list[str], use_llm: bool = True) -> list[ClassificationResult]:
        ...
        if llm_needed:
            for idx, text in zip(llm_indices, llm_needed, strict=False):
