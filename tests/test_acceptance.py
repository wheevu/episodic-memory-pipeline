...
class TestBootstrapDemo:
    ...
    def test_bootstrap_clean_removes_existing(
        ...
        # First bootstrap
        bootstrap_demo(
            fixtures_path=fixtures_path, data_dir=temp_data_dir, use_mock=True, quiet=True
        )
...
class TestDoctorCommand:
    ...
    def test_dry_diagnostics_detects_mock_status(self) -> None:
        ...
        assert result["predictions"]["will_use_mock_embeddings"] is True
        assert result["predictions"]["will_use_mock_llm"] is True
...
class TestEvaluationReproducibility:
    ...
    def test_eval_run_saves_to_disk(self, temp_runs_dir: Path) -> None:
        ...
        result = service.run_evaluation(scenario="diary", precision_k=5, save=True)
